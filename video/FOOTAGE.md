# Видеофоны для `SleepReelFootage`

Положите файлы в `public/footage/` под этими именами и отрендерите `SleepReelFootage`.
Сцена без файла останется на однотонном ночном фоне. Сверху на видео ложится тёмное затемнение, чтобы текст читался, поэтому годятся и светлые кадры.
